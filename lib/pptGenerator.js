...
// ============================================================
// #7 테마 색상 유틸: lightenColor / darkenColor
// ============================================================
...
function addTitle(slide, text, theme, pptx) {
  ...
  // #2 그라데이션 효과 구분선 (primary -> accent fade)
  slide.addShape(pptx.ShapeType.rect, {
    x: 0.8, y: 1.0, w: 1.0, h: 0.03, fill: { color: theme.primary }
  });
  slide.addShape(pptx.ShapeType.rect, {
    x: 1.8, y: 1.0, w: 0.5, h: 0.03, fill: { color: theme.accent }
  });
  slide.addShape(pptx.ShapeType.rect, {
    x: 2.3, y: 1.0, w: 0.3, h: 0.03, fill: { color: theme.accent }, transparency: 40
  });
  slide.addShape(pptx.ShapeType.rect, {
    x: 2.6, y: 1.0, w: 0.2, h: 0.03, fill: { color: theme.accent }, transparency: 70
  });
}
...
/** #8 페이지 번호 개선 - "현재 / 전체" 형식 */
function addPageNumber(slide, num, total, theme) {
...
/** #6 bullet list 렌더링 통합 헬퍼 */
function addBulletList(slide, items, x, y, w, h, theme, opts = {}) {
  const safeItems = Array.isArray(items) ? items : [];
  if (safeItems.length === 0) {
    slide.addText('(항목 없음)', {
      x, y, w, h,
      fontSize: opts.fontSize || 16, fontFace: FONT,
      color: theme.lightText, valign: 'top', italic: true
    });
    return;
  }
  const bullets = safeItems.map(item => ({
    text: typeof item === 'string' ? item : (item.text || item.title || String(item)),
    options: {
      fontSize: opts.fontSize || 20, fontFace: FONT,
      color: opts.color || theme.text,
      bullet: { code: '25CF', color: opts.bulletColor || theme.primary },
      spacing: { before: opts.spacingBefore || 8, after: opts.spacingAfter || 8 }
    }
  }));
...
// #1 마스터 로고 헬퍼
function addMasterLogo(slide, slides) {
  if (slides.masterLogo) {
    slide.addText(slides.masterLogo, {
      x: 10.8, y: 0.12, w: 2.3, h: 0.4,
      fontSize: 10, fontFace: FONT,
      color: '999999', align: 'right', transparency: 30
    });
  }
}

// #10 워터마크 헬퍼
function addWatermark(slide, slides, pptx) {
  if (slides.watermark) {
    slide.addText(slides.watermark, {
      x: 1.5, y: 1.5, w: 10, h: 5,
      fontSize: 72, fontFace: FONT,
      color: '000000', transparency: 92,
...
// #11 footer 헬퍼
function addFooter(slide, slides) {
  if (slides.footer) {
    slide.addText(slides.footer, {
      x: 0.8, y: 7.05, w: 10, h: 0.35,
      fontSize: 8, fontFace: FONT,
      color: 'AAAAAA', align: 'left'
    });
  }
}

// #12 기본 노트 자동 생성 헬퍼
function addAutoNotes(slide, section) {
  if (section.notes) {
    slide.addNotes(section.notes);
  } else if (section.title) {
    slide.addNotes(`이 슬라이드에서는 ${section.title}에 대해 설명합니다.`);
  }
}
...
async function generate(slides, themeName = 'modern', filename = 'presentation.pptx') {
  ...
  // #8 전체 콘텐츠 슬라이드 수 계산 (페이지 번호용)
  const totalContentSlides = slides.content ? slides.content.filter(s => !s.isSection).length : 0;
  ...
  if (slides.cover) {
    ...
    coverSlide.addText(slides.cover.title || '제목 없음', {
    ...
    // #10 워터마크
    addWatermark(coverSlide, slides, pptx);
    // #11 footer
    addFooter(coverSlide, slides);
  }
  ...
  if (slides.toc && slides.toc.length > 0) {
    ...
    // #1 마스터 로고
    addMasterLogo(tocSlide, slides);
    // #10 워터마크
    addWatermark(tocSlide, slides, pptx);
    // #11 footer
    addFooter(tocSlide, slides);
  }
  ...
  if (slides.content && slides.content.length > 0) {
    ...
    for (const section of slides.content) {
      ...
      if (section.isSection) {
        ...
        // #12 스피커 노트 (자동 생성 포함)
        addAutoNotes(sectionSlide, section);
        // #10 워터마크
        addWatermark(sectionSlide, slides, pptx);
        // #11 footer (섹션 슬라이드에는 밝은 색)
        if (slides.footer) {
          sectionSlide.addText(slides.footer, {
            x: 0.8, y: 7.05, w: 10, h: 0.35,
            fontSize: 8, fontFace: FONT,
            color: 'FFFFFF', align: 'left', transparency: 60
          });
        }
      ...
      addTitle(slide, section.title || '(제목 없음)', theme, pptx);

      // #1 마스터 로고
      addMasterLogo(slide, slides);

      // 내용 타입별 처리
      if (section.type === 'bullets') {
        // #6 addBulletList 헬퍼 + #9 빈 데이터 방어
        addBulletList(slide, section.items, 0.8, 1.3, 11.5, 5.5, theme);
        ...
        addBulletList(slide, section.leftItems, 0.8, 1.9, 5.5, 4.5, theme, { fontSize: 14, spacingBefore: 6, spacingAfter: 6 });
        ...
        addBulletList(slide, section.rightItems, 7, 1.9, 5.5, 4.5, theme, { fontSize: 14, spacingBefore: 6, spacingAfter: 6 });
        ...
        // #9 빈 데이터 방어
        if (cards.length === 0) {
          slide.addText('(카드 데이터 없음)', {
            x: 0.8, y: 3, w: 11.5, h: 1,
            fontSize: 16, fontFace: FONT, color: theme.lightText, align: 'center', italic: true
          });
        } else {
        ...
        if (steps.length === 0) {
          ...
          steps.forEach((step, i) => {
            ...
            // 텍스트
            slide.addText(step.title || (typeof step === 'string' ? step : ''), {
        ...
        if (stats.length === 0) {
          ...
          stats.forEach((stat, i) => {
            ...
            // #5 카드 배경 헬퍼
            addCardBg(slide, x, 2.0, statWidth, 3.8, theme, pptx);
            // 숫자
            slide.addText(String(stat.value) + (stat.suffix || ''), {
              x, y: 2.2, w: statWidth, h: 1.3,
              fontSize: 52, fontFace: FONT,
              color: theme.primary, bold: true, align: 'center'
            });
            // #5 비교 아이콘 + 변화값 (stat.change 필드)
            if (stat.change !== undefined && stat.change !== null) {
              const isPositive = Number(stat.change) > 0;
              const changeIcon = isPositive ? '\u25B2' : '\u25BC';
              const changeColor = isPositive ? '22C55E' : 'EF4444';
              const changeText = `${changeIcon} ${Math.abs(Number(stat.change))}${stat.changeSuffix || ''}`;
              slide.addText(changeText, {
                x, y: 3.5, w: statWidth, h: 0.5,
                fontSize: 14, fontFace: FONT,
                color: changeColor, bold: true, align: 'center'
              });
            }
            // 라벨
            slide.addText(stat.label || '', {
              x, y: stat.change !== undefined ? 4.1 : 4.0, w: statWidth, h: 0.8,
              fontSize: 14, fontFace: FONT,
        ...
        if (items.length === 0) {
          ...
          items.forEach((item, i) => {
            ...
            // 값
            slide.addText(String(item.value || 0) + (item.suffix || '%'), {
              x: 9, y, w: 3, h: 0.4,
              fontSize: 16, fontFace: FONT,
            ...
            // 바 채움
            const fillW = Math.max(0.1, (11.5 * Math.min(item.value || 0, 100)) / 100);
            slide.addShape(pptx.ShapeType.roundRect, {
        ...
        addBulletList(slide, section.leftItems, 0.8, 2.2, 4.9, 4.0, theme, { fontSize: 15, spacingBefore: 6, spacingAfter: 6 });
        ...
        addBulletList(slide, section.rightItems, 7.6, 2.2, 4.9, 4.0, theme, { fontSize: 15, bulletColor: theme.secondary, spacingBefore: 6, spacingAfter: 6 });
      ...
      // ========== #8 table 타입 (개선: 셀 여백, 헤더 bold, 경계선) ==========
      } else if (section.type === 'table') {
        const headers = section.headers || [];
        const rows = section.rows || [];
        if (headers.length === 0 && rows.length === 0) {
          slide.addText('(표 데이터 없음)', {
            x: 0.8, y: 3, w: 11.5, h: 1,
            fontSize: 16, fontFace: FONT, color: theme.lightText, align: 'center', italic: true
          });
        } else {
          ...
          headers.forEach((hdr, ci) => {
            slide.addText(String(hdr), {
              x: 0.8 + ci * colW + 0.1, y: startY, w: colW - 0.2, h: rowH,
              fontSize: 14, fontFace: FONT,
              color: 'FFFFFF', bold: true, align: 'center', valign: 'middle'
            });
          });
          ...
          rows.forEach((row, ri) => {
            ...
            // #4 하단 구분선 개선 (더 선명한 경계선)
            slide.addShape(pptx.ShapeType.rect, {
              x: 0.8, y: ry + rowH - 0.015, w: 11.5, h: 0.015,
              fill: { color: theme.lightText }, transparency: 50
            });
            // #4 세로 구분선
            for (let ci = 1; ci < colCount; ci++) {
              slide.addShape(pptx.ShapeType.rect, {
                x: 0.8 + ci * colW - 0.005, y: ry, w: 0.01, h: rowH,
                fill: { color: theme.lightText }, transparency: 60
              });
            }
            (row || []).forEach((cell, ci) => {
              slide.addText(String(cell), {
                x: 0.8 + ci * colW + 0.1, y: ry, w: colW - 0.2, h: rowH,  // #4 셀 내 여백 증가
                fontSize: 13, fontFace: FONT,
      ...
      // #12 스피커 노트 (자동 생성 포함)
      addAutoNotes(slide, section);

      slideNum++;

      // #8 페이지 번호 - "현재 / 전체" 형식
      addPageNumber(slide, slideNum, totalContentSlides, theme);

      // #10 워터마크
      addWatermark(slide, slides, pptx);
      // #11 footer
      addFooter(slide, slides);
    }
  }

  // ========== 마무리 슬라이드 (페이지 번호 없음 #8) ==========
  if (slides.ending) {
    ...
    // #3 QR 코드 안내 (ending.qrText가 있으면)
    if (slides.ending.qrText) {
      // QR placeholder 사각형
      endSlide.addShape(pptx.ShapeType.rect, {
        x: 5.66, y: 5.8, w: 1.2, h: 1.2,
        fill: { color: 'FFFFFF' }, transparency: 30,
        line: { color: 'FFFFFF', width: 1 }
      });
      // QR 내부 장식 (십자형)
      endSlide.addShape(pptx.ShapeType.rect, {
        x: 5.86, y: 6.0, w: 0.25, h: 0.25,
        fill: { color: 'FFFFFF' }, transparency: 50
      });
      endSlide.addShape(pptx.ShapeType.rect, {
        x: 6.21, y: 6.0, w: 0.25, h: 0.25,
        fill: { color: 'FFFFFF' }, transparency: 50
      });
      endSlide.addShape(pptx.ShapeType.rect, {
        x: 5.86, y: 6.35, w: 0.25, h: 0.25,
        fill: { color: 'FFFFFF' }, transparency: 50
      });
      // QR 안내 텍스트
      endSlide.addText(slides.ending.qrText, {
        x: 7.1, y: 5.95, w: 4, h: 0.9,
        fontSize: 12, fontFace: FONT,
        color: 'FFFFFF', transparency: 30, valign: 'middle'
      });
    }

    // #10 워터마크
    addWatermark(endSlide, slides, pptx);
    // #11 footer (밝은 색)
    if (slides.footer) {
      endSlide.addText(slides.footer, {
        x: 0.8, y: 7.05, w: 10, h: 0.35,
        fontSize: 8, fontFace: FONT,
        color: 'FFFFFF', align: 'left', transparency: 60
      });
