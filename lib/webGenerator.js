...
/* ===== 섹션별 악센트 컬러 (시각적 다양성) ===== */
.section-color-0{--sec-accent:${t.primary};--sec-gradient:${t.gradientFrom};--sec-glow:${t.glowColor}}
.section-color-1{--sec-accent:#14b8a6;--sec-gradient:#0d9488;--sec-glow:20,184,166}
.section-color-2{--sec-accent:#a855f7;--sec-gradient:#7c3aed;--sec-glow:168,85,247}
.section-color-3{--sec-accent:#f59e0b;--sec-gradient:#d97706;--sec-glow:245,158,11}
.section-color-4{--sec-accent:#ef4444;--sec-gradient:#dc2626;--sec-glow:239,68,68}

/* 섹션 컬러 적용 */
.slide-section[class*="section-color"]{background:linear-gradient(135deg,var(--sec-gradient),var(--sec-accent))}
.slide-content[class*="section-color"] h2{
  background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient));
  -webkit-background-clip:text;background-clip:text;
}
.slide-content[class*="section-color"] .title-line{background:linear-gradient(90deg,var(--sec-accent),var(--sec-gradient))}
.slide-content[class*="section-color"]::before{background:linear-gradient(90deg,var(--sec-accent),var(--sec-gradient))}
.slide-content[class*="section-color"] .bullet-icon{background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient))}
.slide-content[class*="section-color"] .card::before{background:linear-gradient(90deg,var(--sec-accent),var(--sec-gradient))}
.slide-content[class*="section-color"] .card h3{color:var(--sec-accent)}
.slide-content[class*="section-color"] .card-icon{background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient))}
.slide-content[class*="section-color"] .step-num{background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient));box-shadow:0 4px 20px rgba(var(--sec-glow),0.3)}
.slide-content[class*="section-color"] .stat-value{color:var(--sec-accent)}
.slide-content[class*="section-color"] .stat-circle{stroke:var(--sec-accent)}
.slide-content[class*="section-color"] .hl-icon{background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient))}
...
function showSlide(idx,fromBroadcast){
  ...
  // 내부 애니메이션 요소 리셋
  target.querySelectorAll('.bullet-list li,.card,.step,.stat-item,.col,.tl-item,.checklist li,.quote-mark,.quote-text,.quote-author,.progress-item,.pyramid-level,.ig-item,.donut-item,.matrix-cell,.cycle-node,.chevron,.bar-item,.hl-icon,.hl-main,.hl-sub,.swot-cell,.rm-phase,.comp-side,.comp-vs-badge,.funnel-stage,.toc-item,.check-icon.done svg path,.typewriter-text,.styled-table,.image-text-left,.image-text-right,.agenda-item').forEach(el=>{
...
function generateSlidesHTML(data, t, options = {}) {
  ...
  <div class="slide slide-content" role="tabpanel">
    ...
    <div class="toc-container">
      ${data.toc.map((item, i) => {
        ...
        return `
      <div class="toc-item section-color-${i % 5}" onclick="goToSection(${targetIdx})">
        <div class="toc-num" style="background:linear-gradient(135deg,var(--sec-accent),var(--sec-gradient))"><span>${String(i + 1).padStart(2, '0')}</span></div>
  ...
  if (data.content) {
    for (const s of data.content) {
      if (s.isSection) {
        const ci = s.colorIndex != null ? s.colorIndex : 0;
        html += `
  <div class="slide slide-section section-color-${ci}" role="tabpanel">
    ...
    ${watermarkHTML}
      ...
      const ci = s.colorIndex != null ? ` section-color-${s.colorIndex}` : '';
      html += `
  <div class="slide slide-content${extraClass}${ci}" role="tabpanel">
    <h2>${esc(s.title)}</h2>
    <div class="title-line"></div>`;
